...
## Rustblood integration: keep the editor plugin inert unless it was launched
## through tools/godot_ai_editor.sh. This keeps the runtime helper out of
## ordinary editor sessions and product-facing project configuration.
const RUSTBLOOD_SESSION_ENV := "RUSTBLOOD_GODOT_AI_SESSION"
...
var _export_plugin
## Rustblood integration: only remove the runtime helper if this plugin instance
## created it. A project-owned autoload must survive plugin shutdown unchanged.
var _game_helper_autoload_owned := false
...
func _enter_tree() -> void:
	...
	if _mcp_disabled_for_current_launch():
		_headless_disabled = true
		print("MCP | plugin disabled outside a Godot AI launcher session")
		return
...
func _exit_tree() -> void:
	...
	_stop_server()
	_clear_owned_game_helper_autoload()
...
static func _mcp_disabled_for_current_launch() -> bool:
	if not McpSettings.truthy(OS.get_environment(RUSTBLOOD_SESSION_ENV)):
		return true
...
func _clear_owned_game_helper_autoload() -> void:
	if not _game_helper_autoload_owned:
		return
	var key := "autoload/" + GAME_HELPER_AUTOLOAD_NAME
	var value := "*" + GAME_HELPER_AUTOLOAD_PATH
	if ProjectSettings.get_setting(key, "") == value:
		ProjectSettings.clear(key)
		ProjectSettings.save()
	_game_helper_autoload_owned = false
...
func _ensure_game_helper_autoload() -> void:
	...
	if ProjectSettings.has_setting(key):
		push_warning("MCP: preserving project-owned autoload '%s'" % GAME_HELPER_AUTOLOAD_NAME)
		return
	...
	if err == OK:
		_game_helper_autoload_owned = true
	if err != OK:
